// Needs to const only the sign and verify functions, not the encrypt and decrypt due to additional dependencies. 
const cose = require('./cose-js/sign.js');
// const {createHash as rawHash}= require(;
const rawHash = require("sha256-uint8array").createHash;


const { Certificate, PrivateKey, PublicKey } = require('@fidm/x509');

const zlibDeflate = require('pako').deflate;
const zlibInflate = require('pako').inflate;

const cborDecode = require('cbor').decode;
const cborEncode = require('cbor').encode;
...
const { resolveKey, addCachedCerts, addCachedKeys } = require('./resolver'); 
...
async function sign(payload, publicKeyPem, privateKeyP8) {
  ...
  const cborPayload = cborEncode(payload);
 ...
 */
exports.makeCWT = async function makeCWT(payload, monthsToExpire, issuer) {
  let cwt = new Map();
...
exports.parseCWT = async function parseCWT(cwt) {
...
async function getIssuerKeyId(coseContent) {
  let cborObj = cborDecode(new Uint8Array(coseContent));
  ...
  try {
    let decodedPayload = cborDecode(payload); 
...
async function verifyAndReturnPayload(coseContent, addPublicKeyPem) {
  ...
  const jsonPayload = cborDecode(verified);

  return jsonPayload; 
}

exports.verify = async function verify(coseContent, publicKeyPem) {
  try {
...
async function unpack(uri) {
  ...
  if (data.startsWith(URI_SCHEMA)) {
    ...
    if (data.startsWith(':')) {
      ...
      console.warn("Warning: unsafe HC1: header= require(older versions");
    };
  } else {
      console.warn("Warning: no HC1: header= require(older versions");
  };
  ...
  if (unencodedData[0] == 0x78) {
    unencodedData = zlibInflate(unencodedData);
  }
...
exports.debug = async function debug(uri) {
  return await decodeCbor(await unpack(uri));
}

exports.unpackAndVerify = async function unpackAndVerify(uri, publicKeyPem) {
  try {
...
async function pack32(payload) {
  const zipped = zlibDeflate(payload);
...
exports.pack= async function pack(payload) {
  return await pack45(payload);
}

exports.signAndPack32= async function signAndPack32(payload, publicKeyPem, privateKeyP8) {
  return await pack32(await sign(payload, publicKeyPem, privateKeyP8));
}

exports.signAndPack45= async function signAndPack45(payload, publicKeyPem, privateKeyP8) {
  return await pack45(await sign(payload, publicKeyPem, privateKeyP8));
}

exports.signAndPack= async function signAndPack(payload, publicKeyPem, privateKeyP8) {
  return await pack45(await sign(payload, publicKeyPem, privateKeyP8));
}
